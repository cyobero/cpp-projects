/* This demonstrates a simple event loop.
 * 
 * An event loop is typically an infinite loop such as while (true) that
 * receives messages from the operating system (e.g. clicks, keyboard presses, etc.)
 * and dispatches them one at a time, returning to the loop until an exit
...
class Normal : public Condition {
	public:
		...
		virtual ~Normal() {}
		virtual void log() {
			std::cout << "Logging normal condition ...\n";
		}
};
